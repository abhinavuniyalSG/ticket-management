...
describe("getDashboardOverview: period date ranges", () => {
  ...
  it("'custom' with only a year: the entire year", async () => {
    await DashboardService.getDashboardOverview(requester(), {
      period: DashboardOverviewPeriod.custom,
      year: 2024,
    });

    expect(DashboardRepository.countTicketsByStatus).toHaveBeenCalledWith({
      departmentId: undefined,
      createdFrom: new Date("2024-01-01T00:00:00.000Z"),
      createdTo: new Date("2025-01-01T00:00:00.000Z"),
    });
  });

  it("'custom' with a year and month: the entire month", async () => {
    await DashboardService.getDashboardOverview(requester(), {
      period: DashboardOverviewPeriod.custom,
      year: 2024,
      month: 2,
    ...
    expect(DashboardRepository.countTicketsByStatus).toHaveBeenCalledWith({
      departmentId: undefined,
      createdFrom: new Date("2024-02-01T00:00:00.000Z"),
      createdTo: new Date("2024-03-01T00:00:00.000Z"),
    });
  });

  it("'custom' with year, month, and day: that single day", async () => {
    await DashboardService.getDashboardOverview(requester(), {
      period: DashboardOverviewPeriod.custom,
      year: 2024,
      month: 2,
    ...
    expect(DashboardRepository.countTicketsByStatus).toHaveBeenCalledWith({
      departmentId: undefined,
      createdFrom: new Date("2024-02-29T00:00:00.000Z"),
      createdTo: new Date("2024-03-01T00:00:00.000Z"),
    });
  });

  it("'custom' without a year throws 400", async () => {
    await expect(
      DashboardService.getDashboardOverview(requester(), {
        period: DashboardOverviewPeriod.custom,
      }),
